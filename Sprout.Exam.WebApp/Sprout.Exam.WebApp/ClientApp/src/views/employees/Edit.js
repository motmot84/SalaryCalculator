...
<div className='form-group col-md-6'>
  ...
  <input type='date' className='form-control' id='inputBirthdate4' onChange={this.handleChange.bind(this)} name="birthdate" value={this.state.birthdate} selected={this.state.birthdate} placeholder='Birthdate' />
</div>
</div>
<div className="form-row">
<div className='form-group col-md-6'>
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async getEmployee(id) {
    ...
    const response = await fetch('api/employees/' + id, {
      ...
      this.setState({ id: data.id, fullName: data.fullName, birthdate: bday, tin: data.tin, typeId: data.typeId, loading: false, loadingSave: false });
  }
}
